locals {
  ...
  load_balancer_arn  = "${local.create_alb ? element(concat(local.create_alb_logs ? aws_lb.application.*.arn : aws_lb.application_no_logs.*.arn, list("")),0) : var.load_balancer_arn}"
